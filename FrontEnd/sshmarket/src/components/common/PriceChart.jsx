...
function PriceChart({itemId,chartHeight="400px",orientation = "horizontal"}) {
  ...
  //소매가 차트 옵션
  const[retailChartOptions, setRetailChartOptions] = useState({maintainAspectRatio: false})

  //도매가 차트 옵션
  const[wholesaleChartOptions,setWholeSaleChartOptions] = useState({maintainAspectRatio: false,})
  
  //소매가 품종 드롭다운 리스트 옵션
  const[retailNameDropdownOptions,setRetailNameDropdownOptions] = useState([]);

  //도매가 품종 드롭다운 리스트 옵션
  const[wholesaleNameDropdownOptions,setWholeSaleNameDropdownOptions] = useState([]);

  //소매가 품종 드롭다운 리스트 선택한 옵션
  const[selectedRetailDropdownOption, setSelectedRetailDropdownOption] = useState("50");

  //도매가 품종 드롭다운 리스트 선택한 옵션
  const[selectedWholesaleDropdownOption, setSelectedWholesaleDropdownOption] = useState("50");
                ...
                setWholeSaleChartOptions(
                  {
                    ...
                    plugins: {
                      title: {
                        display: true,
                        fontSize: 30, // <--- this is not a managed option since CHART.JS 3
                        text: `단위 : ${response.data[0].wholesaleUnit =="kg" || response.data[0].wholesaleUnit=="g" ? response.data[0].wholesaleMass.slice(0,-8) : parseInt(response.data[0].wholesaleMass)} ${response.data[0].wholesaleUnit} `
                      }
                    }
                  }
                )
  ...
  return (
    <ChartContainer orientation={orientation}>
      <IndividualChartContainer chartHeight={chartHeight}>    
        <ChartFlexContainer>
          <DropdownContainer>
            <StyledSelect value = {selectedWholesaleDropdownOption} onChange={handleWholesaleDropdownChange}>
              <option value="50" disabled>품종</option>
              {
      ...
      <ChartFlexContainer>
          <DropdownContainer>
            <StyledSelect value = {selectedRetailDropdownOption} onChange={handleRetailDropdownChange}>
              <option value="50" disabled>품종</option>
              {
