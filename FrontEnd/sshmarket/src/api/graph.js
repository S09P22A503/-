...
const getRetailPriceData = async ({ responseFunc, data }) => {
    ...
    try {
      const response = await pyinstance.get(`/products/history/retail?nameId=${nameId}&itemId=${itemId}`)
...
const getWholesalePriceData = async ({ responseFunc, data }) => {
    ...
    try {
      const response = await pyinstance.get(`/products/history/wholesale?nameId=${nameId}&itemId=${itemId}`)
      console.log(response);
      processApiResponse({ responseFunc, response });
      return response;
    } catch (e) {
