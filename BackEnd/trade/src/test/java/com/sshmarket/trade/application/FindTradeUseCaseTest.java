...
class FindTradeUseCaseTest {
    ...
    void 채팅방_조회() {
        ...
        TradeMessage message = sendMessageUseCase.sendMessage(new KafkaMessageDto("안녕", trade.getId(), trade.getBuyerId()));

        Trade trade2 = tradeUseCase.addTrade(new TradeCreateRequestDto(3L, 40L, 10L));
        TradeMessage lastMessage= sendMessageUseCase.sendMessage(new KafkaMessageDto("마지막 메시지", trade2.getId(), trade2.getBuyerId()));
